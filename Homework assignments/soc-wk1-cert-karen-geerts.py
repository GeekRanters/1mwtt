...
end_year = int(input('What is your end year for the leap year exercise? '))
...
# Old-school Roman numerals. In the early days of Roman numerals, the Romans didn’t bother with any of
# this new-fangled subtraction “IX” nonsense.
#
...
#
def roman_numerals_old_style(year):
    result_list = []
    if (year / 1000) >= 1:
        year_thousands = int(str(year)[-4])
        result_list.append(year_thousands * 'M')
    if (year / 100) >= 1:
        year_hundreds = int(str(year)[-3])
        if year_hundreds >= 5:
            result_list.append('D')
            year_hundreds -= 5
        result_list.append(year_hundreds * 'C')
    if (year / 10) >= 1:
        year_tens = int(str(year)[-2])
        if year_tens >= 5:
            result_list.append('L')
            year_tens -= 5
        result_list.append(year_tens * 'X')
    years = int(str(year)[-1])
    if years >= 5:
        result_list.append('V')
        years -= 5
    result_list.append(year_tens * 'I')

    print(''.join(map(str, result_list)))

roman_numerals_old_style(1988)



# “Modern” Roman numerals. Eventually, someone thought it would be terribly
# clever if putting a smaller number before a larger one meant you had to subtract the smaller one.
# As a result of this development, you must now suffer. Rewrite your previous method to return the
# new-style Roman numerals so when someone calls roman_numeral 4, it should return 'IV', 90 should
# be 'XC' etc.


def roman_numerals_new_style(year):
    roman_thousands_list = ['','M','MM','MMM']
    roman_hundreds_list = ['','C','CC','CCC','CD','D','DC', 'DCC', 'DCC', 'CM']
    roman_tens_list = ['','X', 'XX', 'XXX', 'XL', 'L', 'LX', 'LXX', 'LXXX', 'XC']
    roman_units_list = ['','I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX']

    result_list = []

    if year > 1000:
        thousands_idx = int(year/1000)
        result_list.append(roman_thousands_list[thousands_idx])
        year -= 1000 * thousands_idx
    if year > 100:
        hundreds_idx = int(year/100)
        result_list.append(roman_hundreds_list[hundreds_idx])
        year -= 100 * hundreds_idx
    if year > 10:
        tens_idx = int(year/10)
        result_list.append(roman_tens_list[tens_idx])
        year -= 10 * tens_idx

    units_idx = int(str(year)[-1])
    result_list.append(roman_units_list[units_idx])

    print(''.join(map(str, result_list)))

roman_numerals_new_style(1988)
